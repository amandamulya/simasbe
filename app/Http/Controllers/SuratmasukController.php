...
class SuratmasukController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        [
            ...
            'isi.reuired'=>'Kolom Isi Ringkas tidak boleh kosong',
            'file.required'=>'Silahkan pilih file surat',
            'file.mimes'=>'Tipe File harus PDF',
            'file.max'=>'Ukuran file tidak boleh dari 10MB',
        ]);

        if ($request->hasFile('file')) {
            $fileSurat = $request->file('file');
            $newName = 'Neofile' . date('Ymd') . '.' . rand() .'.'. $fileSurat->getClientOriginalExtension();
        ...
        return redirect()->route('suratmasuks.index')->with('success','Data Surat Masuk sudah berhasil disimpan');
    }

    /**
     * Display the specified resource.
     */
    function show(Suratmasuk $suratmasuk)
    {
    ...
    return redirect()->route('suratmasuks.index')->with('success','Import data berhasil');
    }
