...
<div class="card-body">
<form action="{{ route('suratmasuks.update', $suratmasuk->id) }}"
...
Surat" value="{{$suratmasuk->no_surat}}"> 
...
Tgl. Surat" value="{{$suratmasuk->tgl_surat}}"> 
...
@if($errors->has('tglmasuk')) is-invalid @endif" placeholder="Masukkan 
Tgl. Masuk" value="{{$suratmasuk->tgl_masuk}}"> 
...
@if($errors->has('pengirim')) is-invalid @endif" placeholder="Masukkan 
Pengirim Surat" value="{{$suratmasuk->pengirim}}"> 
...
rows="5">{{$suratmasuk->ringkasan}}</textarea>
...
<a href="{{url('suratmasukfile/'.$suratmasuk->file_surat)}}"
    target="_blank" class="btn btn-sm btn-info"><i class="fa fa-file"></i>
    Lihat File</a> 
     @if($errors->has('file'))
     <br>
     <small class="text-danger">
    ...
    <div class="my-2">
     ...
     <a href="{{route('suratmasuks.index')}}" class="btn btn-danger"><i
